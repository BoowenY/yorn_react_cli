...
const createProject = async (project, otherArg) => {
  // 1.提示信息
  log.hint('yang helps you create your project, downloading repo……');
...
const addComponent = async (name, dest) => {
  handleEjsToFile(name, dest, '../templates/react_component.jsx.ejs', `index.jsx`);
}

const addPage = async (name, dest) => {
  addComponent(name, dest);
  handleEjsToFile(name, dest, '../templates/react_router.js.ejs', `${name}Route.js`)
}

const addStore = async (name, dest) => {
  handleEjsToFile(name, dest, '../templates/react_store.js.ejs', 'index.js')
  handleEjsToFile(name, dest, '../templates/react_types.js.ejs', 'types.js')
}
